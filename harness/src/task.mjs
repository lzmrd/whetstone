...
export function loadManifest(path = 'contracts/src/tasks/manifest.json') {
...
export async function prepareTask(manifest) {
  ...
  // proof 2 — the mutation is semantic. A cosmetic mutation leaves a memorised
  // answer correct and the anti-contamination defence is decoration.
  const p2 = await equivalent(task.path, original.path, sig);
  if (p2.equivalent !== false) {
    throw new Error(
  ...
  return { manifest, taskSource, task, baseline, original, proof_1: p1.label, mutation_refuted: true };
}
