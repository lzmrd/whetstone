...
| **Uniswap Foundation** | ~~A pincer on their codebase: v4 math libraries are provable but already squeezed; periphery would have headroom but is not provable, and is guarded by `snapshots/` in CI.~~ ⚠️ **Both halves measured false on 9 September** — see the annotation on [D-07](DECISIONS.md). Four periphery libraries are self-contained MIT with `pure` surface, one of them carries 91% headroom, and v4-core is MIT per file for all its math. What survives from this row is the part that was never about their code: you submit **one project** to multiple tracks, so an impression of oversell travels, and that is why the Uniswap claim is stated as an absolute delta with its equivalence evidence enumerated |
